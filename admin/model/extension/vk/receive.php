...
class ModelExtensionVkReceive extends Model
{
    /**
    ...
    public function addOrder($order)
    {
        $this->db->query(
            "INSERT INTO `" . DB_PREFIX . "order` SET 
                ...
                custom_field = '" . $this->db->escape(isset($order['custom_field']) ? json_encode($order['custom_field']) : '') . "', 
    ...
    /**
     * Add order products in opencart
     *
     * @param int $order_id
    ...
    /**
     * Get country by name from opencart
     *
     * @param string $name
     *
     * @return array
    ...
    /**
     * Get zone by name from opencart
     *
     * @param string $name
     *
     * @return array
    ...
    /**
     * Get product option value from opencart
     *
    ...
    public function editOrder($order_id, $order)
    {
        $this->db->query(
            "UPDATE `" . DB_PREFIX . "order` SET 
                ...
                email = '" . $this->db->escape($order['email']) . "', 
                telephone = '" . $this->db->escape($order['telephone']) . "', 
                custom_field = '" . $this->db->escape(json_encode($order['custom_field'])) . "', 
                payment_firstname = '" . $this->db->escape($order['payment_firstname']) . "', 
                payment_lastname = '" . $this->db->escape($order['payment_lastname']) . "',  
                payment_address_1 = '" . $this->db->escape($order['payment_address_1']) . "', 
    ...
    /**
     * Edit order status in opencart
     *
     * @param int $order_id
     * @param int $status_id
     */
    public function editOrderStatus($order_id, $status_id)
    {
        $this->db->query(
            "UPDATE `" . DB_PREFIX . "order` SET 
